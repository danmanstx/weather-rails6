require 'rails_helper'

RSpec.describe ForecastsController, type: :routing do
  describe 'routing' do
    it 'routes to #index' do
      expect(get: '/forecasts').to route_to('forecasts#index')
    end

    it 'routes to #new' do
      expect(get: '/forecasts/new').to route_to('forecasts#new')
    end

    it 'routes to #show' do
      expect(get: '/places/1/forecasts/1').to route_to('forecasts#show', place_id: '1', id: '1')
    end

    it 'routes to #edit' do
      expect(get: '/places/1/forecasts/1/edit').to route_to('forecasts#edit', place_id: '1', id: '1')
    end


    it 'routes to #create' do
      expect(post: '/places/1/forecasts').to route_to('forecasts#create', place_id: '1')
    end

    it 'routes to #update via PUT' do
      expect(put: '/forecasts/1').to route_to('forecasts#update', id: '1')
    end

    it 'routes to #update via PATCH' do
      expect(patch: '/forecasts/1').to route_to('forecasts#update', id: '1')
    end

    it 'routes to #destroy' do
      expect(delete: '/forecasts/1').to route_to('forecasts#destroy', id: '1')
    end
  end
end
